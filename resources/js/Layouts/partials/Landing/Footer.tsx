import { AppSetting, LandingData } from "@/types";
...
type DataStateProp = {
  logo: string | null;
  usefulLink: UsefulLinkProp[] | null;
  socialMedia: SocialMediaProp[] | null;
};
...
export default function Footer() {
  ...
  const initialState: DataStateProp = {
    logo: null,
    usefulLink: null,
    socialMedia: null,
  };

  const { logo, socialMedia, usefulLink } = footer.reduce((acc, item) => {
    switch (item.key) {
      case "banner":
        acc.logo = item.value;
        break;
      case "social.media":
        acc.socialMedia = JSON.parse(item.value);
        break;
      case "external.link":
        acc.usefulLink = JSON.parse(item.value);
        break;
      default:
        break;
    }
  ...
  return (
    <div className="bg-base-200">
      <footer className="footer text-base-content p-10 w-[75%] lg:ms-[20vw] ms-[10vw]">
        <aside className="lg:max-w-[25vw]">
          <div className="flex justify-center">
            <Link href={route("landing")} className="btn-ghost">
              {logo && logo !== "" ? (
                <img
                  src={logo}
                  alt="App Banner Logo"
          ...
          <p className="text-start mt-5 font-extrabold">Hubungi kami</p>
          <RenderSocialMedia />
        </aside>
        <nav className="lg:max-w-[15vw]">
          <h6 className="footer-title font-extrabold">Navigasi</h6>
          <Link
            href={route("about")}
            preserveScroll
            className="link link-hover"
